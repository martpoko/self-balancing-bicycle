...
const uint16_t PWMVALUE = F_CPU / PWM_FREQUENCY / 2;  // Prescale for new pwm frequency
                      ...
                      //Alternative Tunings
float P1 = 49.0;      //49.0  / 55.0 / defaults 75
float I2 = 4.80;      //4.80  / 5.83 / defaults 5.25
float D3 = 0.085;     //0.085 / 0.07 / defaults 0.04
...
bool driving_servo_set = false;   // Driving state for routine
bool steering_servo_set = false;  // Steering state for routine
...
void loop() {   // Main Loop
  ...
  if (currentT - previousT_1 >= loop_time) {
    ...
    if (vertical) {
      ...
      pwm_s = -constrain(P1 * robot_angle + I2 * gyroZfilt + D3 * -motor_speed, -255, 255); 
      ...
      driving_servo.write(DRIVING_CENTER);    // Disable driving servo
